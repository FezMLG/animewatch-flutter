...
class SeriesArguments {
  final int id;
  final String title;

  SeriesArguments({required this.title, required this.id});
}
...
class SeriesScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final args = ModalRoute.of(context)!.settings.arguments as SeriesArguments;

    return DefaultTabController(
      length: 2,
      child: FutureBuilder<SeriesDetails>(
        future: AniList().getDetailsOfSeries(id: args.id),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const LoadingScreen();
          } else if (snapshot.hasError) {
